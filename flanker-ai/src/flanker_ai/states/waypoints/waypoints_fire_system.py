...
@dataclass
class DoublePinAvoidanceConfig:
    avoids_double_pins: bool = False
...
class WaypointsFireSystem(FireSystem):
    ...
    def apply_fire_outcome(
        ...
        avoids_double_pins: bool = False
        for _, conf in gs.query(DoublePinAvoidanceConfig):
            avoids_double_pins = conf.avoids_double_pins
            break

        if avoids_double_pins == False:
            FireSystem.apply_fire_outcome(gs, target_id, fire_outcome)
        else:
