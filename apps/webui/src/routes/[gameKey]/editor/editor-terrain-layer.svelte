	...
	{#each props.controller.mapData.terrains as terrain}
		...
		<g onclick={() => selectTerrain(terrain)}>
			{#if ['FOREST', 'FIELD', 'WATER'].includes(terrain.terrainType)}
				<path
					d={GetSmoothedClosedPath(vertices, 0.7)}
					class="closed-terrain {selectedClass}"
				/>
			{:else if terrain.terrainType == 'ROAD'}
				<path
					d={GetSmoothedPath(vertices, 0.7)}
					class="open-terrain {selectedClass}"
				/>
			{:else if terrain.terrainType == 'BUILDING'}
				<path
					d={GetClosedPath(vertices)}
					class="closed-terrain {selectedClass}"
				/>
			{/if}
		</g>
