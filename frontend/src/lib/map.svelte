<script lang="ts">
	...
	let { props: props = $bindable() }: { props: Props } = $props();
	...
	onMount(async () => {
		props.terrainData = await GetTerrainData();
		props.unitData = await GetRifleSquadsData();
	...
	async function ConfirmMarker(_: MouseEvent) {
		...
		if (selectedUnit !== null && marker !== null) {
			props.unitData = await MoveRifleSquad(selectedUnit, marker);
			marker = null;
		}
	}
...
>
	<g bind:this={zoomLayer}>
		{#each props.terrainData as terrainFeatureData}
			<TerrainFeature featureData={terrainFeatureData} />
		{/each}

		{#each props.unitData as unit}
