...
class AssaultSystem:
    ...
    def _get_assault_outcome(
        ...
        if attacker_roll <= threshold:
            return AssaultControls.Outcomes.SUCCESS
        else:
            return AssaultControls.Outcomes.FAIL
    ...
    def assault(
        ...
        # Once at location, do dice roll; only one can survive
        outcome = AssaultSystem._get_assault_outcome(gs, attacker_id, target_id)
        match outcome:
