"""
Geometry System for 2D polygons & computing intersections.
"""
...
class PolygonSpace:
    ...
    polygons: list[Polygon] = []  # List of 2D numpy arrays, each representing a feature
    ...
    @staticmethod
    def get_intersects(start: Vec2, end: Vec2, mask: int = -1) -> Iterator[Intersection]:
        """ Returns an `Iterator` of intersection points between the line segment and features."""
        for feature in PolygonSpace.polygons:
            if feature.flag & mask:
                for b1, b2 in pairwise(feature.vertices):
                    if (intsct := PolygonSpace._get_intersect(start, end, b1, b2)) is not None:
